...
public class ArmConfiguration {
        ...
        public static double joint1AngleToEncoder(double angle) {
                ...
                double c = Math.sqrt(Math.pow(Arm.ARM_JOINT_1_PIVOT_TO_LEADSCREW_LENGTH, 2) + Math.pow(Arm.ARM_JOINT_1_PIVOT_TO_MOTOR_LENGTH, 2)
                                - (2 * Arm.ARM_JOINT_1_PIVOT_TO_LEADSCREW_LENGTH * Arm.ARM_JOINT_1_PIVOT_TO_MOTOR_LENGTH) * Math.cos(theta));
        ...
        public static double joint2AngleToEncoder(double angle) {
                ...
                double c = Math.sqrt(Math.pow(Arm.ARM_JOINT_2_PIVOT_TO_LEADSCREW_LENGTH, 2) + Math.pow(Arm.ARM_JOINT_2_PIVOT_TO_MOTOR_LENGTH, 2)
                                - (2 * Arm.ARM_JOINT_2_PIVOT_TO_LEADSCREW_LENGTH * Arm.ARM_JOINT_2_PIVOT_TO_MOTOR_LENGTH) * Math.cos(theta));
        ...
        public ArmConfiguration(double turretPosition, double firstJointPosition, double secondJointPosition, double thirdJointPosition, double wristPosition, POSITION_TYPE positionType) {
                ...
                Translation2d linkage2 = new Translation2d(Arm.ARM_LINKAGE_2_LENGTH, Rotation2d.fromRadians((Math.PI / 2) - getSecondJointPosition(POSITION_TYPE.ANGLE)))
                                .plus(linkage3.rotateBy(Rotation2d.fromRadians(-getSecondJointPosition(POSITION_TYPE.ANGLE))));
                Translation2d linkage1 = new Translation2d(Arm.ARM_LINKAGE_1_LENGTH, Rotation2d.fromRadians((Math.PI / 2) - getFirstJointPosition(POSITION_TYPE.ANGLE)))
                                .plus(linkage2.rotateBy(Rotation2d.fromRadians(-getFirstJointPosition(POSITION_TYPE.ANGLE))));
        ...
        public static ArmConfiguration fromEndPosition(Positions.Pose3d endPose, double grabberAngleRadians, double wristRotationRadians) {
                ...
                Translation2d pointToGrab2d = endPosition.rotateBy(new Rotation3d(0, 0, -angleToPoint)).rotateBy(new Rotation3d(-Math.PI / 2, 0, 0)).toTranslation2d()
                                .plus(new Translation2d(0, -Arm.ARM_LINKAGE_0_LENGTH)).plus(new Translation2d(Arm.ARM_LINKAGE_3_LENGTH, new Rotation2d(Math.PI - grabberAngleRadians)));
        ...
        public static double joint1EncoderToAngle(double rotations) {
                ...
                // System.out.println(Math.toDegrees((Math.PI/2) - Math.acos(numerator /
                // denominator) +
                // Arm.ARM_JOINT_1_PIVOT_TO_MOTOR_HORIZONTAL_ANGLE_OFFSET));
        ...
        public FFData getJoint1FFData() {
                ...
                Positions.Pose3d pastJ2CG = ArmConfiguration.combineCG(Positions.Pose3d.fromRobotSpace(linkage2CG), Positions.Pose3d.fromRobotSpace(linkage3CG), Arm.ARM_LINKAGE_2_MASS,
                                Arm.ARM_LINKAGE_3_MASS);

                Positions.Pose3d pastJ1CG = ArmConfiguration.combineCG(pastJ2CG, Positions.Pose3d.fromRobotSpace(linkage1CG), Arm.ARM_LINKAGE_3_MASS + Arm.ARM_LINKAGE_2_MASS, Arm.ARM_LINKAGE_1_MASS);

                double joint1CGDistance = joint1Pose3d.minus(pastJ1CG.inRobotSpace()).getTranslation().getNorm();
                double joint1CGAngle = Math.atan((joint1Pose3d.getZ() - pastJ1CG.inRobotSpace().getZ())
                                / Math.sqrt(Math.pow(joint1Pose3d.getX() - pastJ1CG.inRobotSpace().getX(), 2) + Math.pow(joint1Pose3d.getY() - pastJ1CG.inRobotSpace().getY(), 2)));
        ...
        public FFData getJoint2FFData() {
                ...
                double joint2CGAngle = Math.atan((joint2Pose3d.getZ() - pastJ2CG.inRobotSpace().getZ())
                                / Math.sqrt(Math.pow(joint2Pose3d.getX() - pastJ2CG.inRobotSpace().getX(), 2) + Math.pow(joint2Pose3d.getY() - pastJ2CG.inRobotSpace().getY(), 2)));
        ...
        public FFData getJoint3FFData() {
                ...
                double joint3CGAngle = Math.atan(
                                (joint3Pose3d.getZ() - linkage3CG.getZ()) / Math.sqrt(Math.pow(joint3Pose3d.getX() - linkage3CG.getX(), 2) + Math.pow(joint3Pose3d.getY() - linkage3CG.getY(), 2)));
        ...
        public String toString() {
                return "Turret Encoder: " + turretPositionRotations + "\nFirst Joint Encoder: " + firstJointPositionRotations + "\nSecond Joint Encoder: " + secondJointPositionRotations
                                ...
                                + String.format("\nJoint 1 Pos (RS): (%.2f, %.2f, %.2f)", getJoint1Pose().inRobotSpace().getX(), getJoint1Pose().inRobotSpace().getY(),
                                                getJoint1Pose().inRobotSpace().getZ())
                                + String.format("\nJoint 2 Pos (RS): (%.2f, %.2f, %.2f)", getJoint2Pose().inRobotSpace().getX(), getJoint2Pose().inRobotSpace().getY(),
                                                getJoint2Pose().inRobotSpace().getZ())
                                + String.format("\nJoint 3 Pos (RS): (%.2f, %.2f, %.2f)", getJoint3Pose().inRobotSpace().getX(), getJoint3Pose().inRobotSpace().getY(),
                                                getJoint3Pose().inRobotSpace().getZ())
                                + String.format("\nLinkage 1 CG (RS): (%.2f, %.2f, %.2f)", getLinkage1CG().inRobotSpace().getX(), getLinkage1CG().inRobotSpace().getY(),
                                                getLinkage1CG().inRobotSpace().getZ())
                                + String.format("\nLinkage 2 CG (RS): (%.2f, %.2f, %.2f)", getLinkage2CG().inRobotSpace().getX(), getLinkage2CG().inRobotSpace().getY(),
                                                getLinkage2CG().inRobotSpace().getZ())
                                + String.format("\nLinkage 3 CG (RS): (%.2f, %.2f, %.2f)", getLinkage3CG().inRobotSpace().getX(), getLinkage3CG().inRobotSpace().getY(),
                                                getLinkage3CG().inRobotSpace().getZ());
        }
        ...
        public boolean equals(ArmConfiguration secondConfig, double tolerance) {
                double distance = endPose.inRobotSpace().minus(secondConfig.getEndPosition().inRobotSpace()).getTranslation().getNorm();
                // System.out.println(distance);
                return distance < tolerance;
        }

        /**
         * Returns wheather or not this configuration is valid and within the soft limits If out of the soft limits but moving back towards the soft limits, it is still valid
         * 
        ...
        public boolean withinDistance() {
                ...
                if (joint3Pose.getZ() > Arm.MAX_HEIGHT || joint3Pose.getZ() < 0) {
                        return false;
                }
                return true;
        }
