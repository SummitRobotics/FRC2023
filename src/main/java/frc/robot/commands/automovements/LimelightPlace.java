...
public class LimelightPlace extends CommandBase {
    ...
    public void execute() {
        if (tv.getDouble(0) == 1) {
            drivetrain.setBothMotorPower(arm.getCurrentArmConfiguration().getTurretPosition(POSITION_TYPE.ANGLE) > 0 ? -1 : 1 * pidController.calculate(tx.getDouble(0.0) + arm.getCurrentArmConfiguration().getTurretPosition(POSITION_TYPE.ANGLE) > 0 ? 12 : -12));
        }
    }
